...
export class PuchaseProductModal extends Component<IProps, IState> {
    ...
    isPurchaseButtonEnabled(): boolean {
        var enabled: boolean = true;

        for (var i = 0; i < this.state.productAttributes.length; i++) {
            let attr = this.state.productAttributes[i];

            if (attr.is_required) {
                let found: boolean = false;

                for (let j = 0; j < this.state.activeAttributes.length; j++) {
                    let attrVal = this.state.activeAttributes[j];

                    if (attrVal.attributeIndex == i) {
                        found = true;
                    }
                }
    ...
    render(): JSX.Element {
        ...
        let purchaseEnabled = this.isPurchaseButtonEnabled();
        let purchaseEnabledStyle: ViewStyle = { backgroundColor: "#ff0006ff" };
        let purchaseDisabledStyle: ViewStyle = { backgroundColor: "#ff000666" };
                        ...
                        <View key="PurchaseButton" style={styles.purchaseButtonArea}>
                            <TouchableOpacity
                                ...
                                style={[
                                    ...
                                    purchaseEnabled ? purchaseEnabledStyle : purchaseDisabledStyle
                                ]}
                                onPress={() => {
                                    this.props.purchase(
